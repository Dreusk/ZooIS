...
a.substr(d+1):a}return"string"===typeof a?a.replace(/\r/g,""):a},check:function(a){a=this.validationTargetFor(this.clean(a));var b=c(a).rules(),d=c.map(b,function(k,m){return m}).length,e=!1,f=this.elementValue(a),h;if("function"===typeof b.normalizer)var g=b.normalizer;else"function"===typeof this.settings.normalizer&&(g=this.settings.normalizer);if(g){f=g.call(a,f);if("string"!==typeof f)throw new TypeError("The normalizer should return a string value.");delete b.normalizer}for(h in b){g={method:h,
parameters:b[h]};try{var l=c.validator.methods[h].call(this,f,a,g.parameters);if("dependency-mismatch"===l&&1===d)e=!0;else{e=!1;if("pending"===l){this.toHide=this.toHide.not(this.errorsFor(a));return}if(!l)return this.formatAndAdd(a,g),!1}}catch(k){throw this.settings.debug&&window.console&&console.log("Exception occurred when checking element "+a.id+", check the '"+g.method+"' method.",k),k instanceof TypeError&&(k.message+=".  Exception occurred when checking element "+a.id+", check the '"+g.method+
"' method."),k;}}if(!e)return this.objectLength(b)&&this.successList.push(a),!0},customDataMessage:function(a,b){return c(a).data("msg"+b.charAt(0).toUpperCase()+b.substring(1).toLowerCase())||c(a).data("msg")},customMessage:function(a,b){return(a=this.settings.messages[a])&&(a.constructor===String?a:a[b])},findDefined:function(){for(var a=0;a<arguments.length;a++)if(void 0!==arguments[a])return arguments[a]},defaultMessage:function(a,b){"string"===typeof b&&(b={method:b});var d=this.findDefined(this.customMessage(a.name,
...
(n[b]&&n[b].abort(),n[b]=d)});else{var p=c.ajax;c.ajax=function(a){var b=("port"in a?a:c.ajaxSettings).port;return"abort"===("mode"in a?a:c.ajaxSettings).mode?(n[b]&&n[b].abort(),n[b]=p.apply(this,arguments),n[b]):p.apply(this,arguments)}}return c});
